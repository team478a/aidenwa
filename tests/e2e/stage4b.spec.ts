...
test('Stage 4B-1 remains fail-closed and exposes only the single-call limited-test UI', async ({
  ...
  const twilioStatus = page.locator('.connection-item').filter({ hasText: 'Twilio' });
  await expect(twilioStatus).toContainText(/disabled|not_configured/);
  const zoomPhoneStatus = page.locator('.connection-item').filter({ hasText: 'Zoom Phone' });
  await expect(zoomPhoneStatus).toContainText('未接続');
  await expect(zoomPhoneStatus).toContainText('Stage 4B-2以降');
  await expect(page.getByRole('heading', { name: '1件ずつ手動発信' })).toBeVisible();
  await expect(page.getByText('本日の実電話予約', { exact: true })).toBeVisible();
  await expect(page.getByText('現在の通話数', { exact: true })).toBeVisible();
