...
export function validateScenario(nodes: GraphNode[], edges: GraphEdge[]) {
  ...
  if (starts[0]) {
    ...
    if (maximumDepth > MAX_SCENARIO_DEPTH)
      errors.push(`maximum_depth_exceeded:${maximumDepth}:${MAX_SCENARIO_DEPTH}`);
  ...
  for (const node of nodes) {
    ...
    if (
      ...
      (typeof config.maxCycles !== 'number' ||
        ...
        config.maxCycles < 1 ||
        config.maxCycles > MAX_CYCLE_COUNT)
    )
      errors.push(`invalid_max_cycles:${node.nodeKey}:${MAX_CYCLE_COUNT}`);
