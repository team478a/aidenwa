...
export function registerStage4ERoutes(app: FastifyInstance, deps: Deps) {
  const { prisma, env } = deps;
  const roles = [UserRole.admin, UserRole.manager, UserRole.sales];
  const mutate = async (request: FastifyRequest, reply: FastifyReply, allowed = roles) => {
  ...
  app.post('/api/v1/availability-rules', async (request, reply) => {
    ...
    if (auth.role === UserRole.sales && input.userId !== auth.userId)
  ...
  app.get('/api/v1/appointments/:id', async (request, reply) => {
    ...
    return {
      appointment,
      events: await prisma.appointmentEvent.findMany({
        where: { organizationId: auth.organizationId, appointmentId: id },
  ...
  app.post('/api/v1/appointments/slots', async (request, reply) => {
    ...
    if (auth.role === UserRole.sales && input.assigneeUserId !== auth.userId)
